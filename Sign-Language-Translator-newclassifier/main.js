...
class Main {
  ...
  if (!this.predButton) {
    console.error("Translate button not found!"); // Debugging: Check if the button exists
  } else {
    console.log("Translate button found:", this.predButton); 
  }
  ...
  initializeTranslator() {
    ...
    this.loadKNN();
    this.loadMetadata(); // Load metadata on startup
  }
  ...
  train(gestureIndex) {
    ...
    if (this.videoPlaying) {
      ...
      if (exampleCount > 0) {
        ...
        this.saveImageToFile(this.video, words[gestureIndex], exampleCount);

        // Save metadata
        this.saveMetadata(words[gestureIndex], exampleCount);
  ...
  /*This function saves the webcam image as a file in the `images` folder.*/
  saveImageToFile(imageData, gestureName, exampleCount) {
    const canvas = document.createElement('canvas');
    canvas.width = IMAGE_SIZE;
    canvas.height = IMAGE_SIZE;
    const ctx = canvas.getContext('2d');
    ctx.drawImage(imageData, 0, 0, IMAGE_SIZE, IMAGE_SIZE);
    ...
    link.href = dataURL;
    link.download = `images/${gestureName}_${exampleCount}.png`; // Unique filename
    link.click(); // Trigger the download
  }

  /*This function saves metadata (gesture names and example counts) to localStorage.*/
  saveMetadata(gestureName, exampleCount) {
    ...
    // Check if the gesture already exists in metadata
    const gestureData = metadata.find((data) => data.gesture === gestureName);
    if (gestureData) {
      // Update the example count for the existing gesture
      gestureData.exampleCount = exampleCount;
    } else {
      // Add a new entry for the gesture
      metadata.push({ gesture: gestureName, exampleCount });
    }
  ...
  /*This function loads metadata from localStorage and updates the UI.*/
  loadMetadata() {
    if (localStorage.getItem('metadata')) {
      const metadata = JSON.parse(localStorage.getItem('metadata'));
      metadata.forEach((data) => {
        const gestureIndex = words.indexOf(data.gesture);
        if (gestureIndex !== -1) {
          // Update the example count display
          this.exampleCountDisplay[gestureIndex].innerText = ` ${data.exampleCount} examples`;

          // Update the kNN classifier's example count
          this.knn.setClassExampleCount(gestureIndex, data.exampleCount);
        }
      });
    }
  }
  ...
  createTranslateBtn() {
    console.log("Creating Translate button..."); // Debugging: Confirm this method is called
  
    // Ensure the predButton is referenced correctly
    if (!this.predButton) {
      console.error("Translate button not found!"); // Debugging: Check if the button exists
      return;
    }
    ...
    this.predButton.addEventListener('click', () => {
      console.log("Translate button clicked"); // Debugging: Check if this logs in the console
      const exampleCount = this.knn.getClassExampleCount();
      console.log("Example Count:", exampleCount); // Debugging: Check example counts
